/*    Lunes, 30 de Noviembre de 2.009  -   v0.00.260.sql 

	Agregamos los items: Codigo1erNivel, 2do, 3er, ... 
	a la tabla tTempWebReport_PresupuestoConsultaMensual 
...
IF EXISTS(SELECT * FROM dbo.tTempWebReport_PresupuestoConsultaMensual)
	 EXEC('INSERT INTO dbo.Tmp_tTempWebReport_PresupuestoConsultaMensual (CiaContab, Moneda, AnoFiscal, MesFiscal, MesCalendario, NombreMes, CodigoPresupuesto, FactorConversion, MontoEstimado, MontoEjecutado, Variacion, MontoEstimadoAcum, MontoEjecutadoAcum, VariacionAcum, NombreUsuario)
		SELECT CiaContab, Moneda, AnoFiscal, MesFiscal, MesCalendario, NombreMes, CodigoPresupuesto, FactorConversion, MontoEstimado, MontoEjecutado, Variacion, MontoEstimadoAcum, MontoEjecutadoAcum, VariacionAcum, NombreUsuario FROM dbo.tTempWebReport_PresupuestoConsultaMensual WITH (HOLDLOCK TABLOCKX)')
GO
...
Insert Into tVersionWeb (VersionActual, Fecha) Values('v0.00.260', GetDate()) 